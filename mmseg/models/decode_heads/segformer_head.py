...
class SegFormerHead(BaseDecodeHead):
    ...
    def forward(self, inputs, **kwargs):
        if kwargs.get("mood", None) is not None:
            return self.forward_mood(inputs, **kwargs)

        if kwargs.get('module', None):
            return self.forward_module(inputs, module=kwargs['module'])
    ...
    def forward_mood(self, inputs, **kwargs):
        x = self._transform_inputs(inputs)  # len=4, 1/4,1/8,1/16,1/32
        c1, c2, c3, c4 = x
        ...
        x_cls = self.linear_pred(x_feat)

        return x_cls, x_feat

    def forward_module(self, inputs, module):
        assert module == 1
        ...
        _c1 = _c1.reshape(n, -1, c1.shape[2], c1.shape[3])
        _c[0] = _c1
