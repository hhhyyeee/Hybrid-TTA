...
class Attention(nn.Module, AdapterMixin):
    ...
    def forward(self, x, H, W):
        ...
        # attn_sc = (q @ k.transpose(-2, -1).contiguous()) * self.scale
        # attn = attn_sc.softmax(dim=-1)
...
class Block(nn.Module, AdapterMixin):
    ...
    def forward(self, x, H, W):
        kwargs = {"H": H, "W": W}
        # attn, _ = self.attn(self.norm1(x), H, W)
...
class MixVisionTransformerCVP(nn.Module):
    ...
    def __init__(self,
        ...
        self.clf_flag = False #!DETERMINED
        if self.clf_flag:
        ...
        # VPT config
        self.vpt_flag = True if cfg["tuning_stage"] is not None else False
        if self.vpt_flag:
            ...
            self.embedding_tune = False
            self.conv_tune = False
        ...
        # def get_attention_score(name):
        #     def hook_fn(module, inp, out):
        #         if len(out) > 1:
        #             self.attn_score_dict[name] = out[1].detach().cpu()
        #     return hook_fn
        # for name, module in self.named_modules():
        #     if 'attn' in name:
        #         self.hooks[name] = module.register_forward_hook(get_attention_score(name))
    ...
    def forward_module(self, x, module=1): #!DEBUG
        assert module <= 4
        B = x.shape[0]
        outs = []
