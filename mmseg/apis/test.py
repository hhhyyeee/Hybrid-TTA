...
def single_gpu_our(model,
    ...
    for i, data in enumerate(data_loader):
        ...
        wandb_log.update({'grad.total_norm': total_norm})

        if freeze_ctrl.mode == 'contextual_thompson_sampling':
            freeze_ctrl(model, epoch_index=epoch_index, iter_index=i, loss=log_vars, total_norm=total_norm)
            wandb_log.update({
                'freeze_ctrl.frozen_flag': int(freeze_ctrl.frozen_flag),
                'freeze_ctrl.wma_loss': freeze_ctrl.wma_loss,
                'freeze_ctrl.reward': freeze_ctrl.reward
            })
