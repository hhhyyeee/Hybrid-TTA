...
class CustomDataset(Dataset):
    ...
    def __init__(
        ...
        self.img_infos = sorted(self.img_infos, key=lambda d: d['filename']) #!DEBUG
    ...
    def evaluate(self,
            ...
            eval_results = {}
            gt_seg_maps = self.get_gt_seg_maps(efficient_test)
            gt_seg_maps = [cv2.resize(_gt, (self.gt_shape[1], self.gt_shape[0]), interpolation=cv2.INTER_CUBIC) for _gt in gt_seg_maps] #!DEBUG
            if self.CLASSES is None:
                num_classes = len(
